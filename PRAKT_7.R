#1.Zakon za golemite chisla
#Ako imame populaciq(zarcheta), pri bezkrainost ochakvame, che veroqtnosta e 1/6
#ako imame 1 opit, to ili e 1 ili 0, s 2 opita e g/d sushtoto, no ako pravim n opita, to -> 1/6
x = sample(1:6, 10, replace = TRUE)
table(x)
table(x)/length(x) #veroqtnosti
barplot(table(x))
#oshte takiva sus stepeni
...
qqplot.das(babies$wt)

#Task2)
iq
qqnorm(iq)
qqline(iq)
##tejki opashki mediana
