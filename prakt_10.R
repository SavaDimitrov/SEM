# A - izvadka ot populaciq
# XA(gorna cherta) = ?const
# 
# B - izvadka ot populaciq
# XB(gorna cherta) = ?const2

#library(StatDA)
#qplot.das - proverka za tejki opashki
#shapiro.test()

#prop.test() - proporcii za doveritelni intervali
...
wilcox.test(ewr_AA, ewr_NW, conf.level = 0.95, alternative = "two.sided")
...
#check for normality
library(StatDA)
qqplot.das(homework$Private)
qqplot.das(homework$Public)
#shapiro test for normality
shapiro.test(homework$Private)
#p-value = 0.8606 > alpha = 0.05 => accept H0
shapiro.test(homework$Public)
#p-value = 0.9999 > alpha = 0.05 => accept H0
#both have normal distribution, so we can use t.test

#11.2
library(UsingR)
corn
#Do a two-sample t-test on the date
#h0: muNew = muStandart
#h1: muNew != muStandart
t.test(corn$New, corn$Standard, paired = TRUE)
#p-value = 0.00279 < alpha = 0.05 => reject H0
#do the assumption seems to be met
