...
## Examples:
## Dice
a = sample(1:6, size = 10, replace = TRUE)
table(a)
prop.table(table(a))
...
## Pick a card
cards = paste(rep(c("A",2:100,"J","Q","K"), 4),
              c("H","D","S","C"))
sample(x = cards, size = 5)
sample(x = cards, size = 100)
...
#From here on - Lecture 5 - 110 line
...
######################TASKS######################
###Task 1
# Do two simulations of a binomial number with n = 100 and p = .5. Do you get the same results each time?
...
###Task 2 - Zarche(ravnomerno razpredelenie)
dice = sample(1:6, size = 100, replace = TRUE)
table(dice)
prop.table(table(dice))
###Binomno razpr.(Bernuli)
###"Uspeh" = 2; Y ~ Bi(100,1/7); stoinost okolo 14,29
a = rbinom(1000, 100, 1/7)
table(a)
hist(a, probability = TRUE)
points(1:100, dbinom(1:100, 100, 1/7), type = "h", lwd = 3)
qbinom(14,100,1/7)
rgeom(1000, 1/7)